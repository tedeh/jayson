...
var Method = module.exports = function(handler, options) {
  ...
  this.handler =
    _.isArray(handler || options.handler) ?
...
Method.prototype.setHandler = function(handler) {
  this.handler = _.isArray(handler) ? handler : [handler];
};
...
Method.prototype.execute = function(server, params, callback) {
  ...
  if(options.collect) {
    ...
    if(objectParams) {
      ...
      var parameters = _.initial(utils.getParameterNames(handler[index]));
    ...
    if (args.length != handler[index].length) {
  ...
  function next(err, result) {
    ...
    if(options.collect) {
      return stack.call(server, callParams, next);
    } else {
      return stack.apply(server, args);
    }
  }
