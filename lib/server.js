 ...
 *  @api public
 */
Server.prototype.errorResponse = function(code, message, data) {
...
Server.prototype.call = function(request, callback) {
  ...
  try {
    ...
    return callback(utils.response(this.errorResponse(Server.errors.PARSE_ERROR)));
  }
  ...
  if(Array.isArray(request)) {
    ...
    if(request.length === 0) {
      return callback(utils.response(this.errorResponse(Server.errors.INVALID_REQUEST)));
    }
  ...
  // from now on we are "notification/id"-aware and can ignore errors for such requests
  ...
  if(!this.hasMethod(request.method)) {
    return respond(this.errorResponse(Server.errors.METHOD_NOT_FOUND));
  }
  ...
  args.push(function(error, result) {
    ...
    if(error || (!error && !result)) {
      return respond(this.errorResponse(Server.errors.INTERNAL_ERROR));
    }
...
Server.prototype._batch = function(requests, callback) {
  ...
  var wrapper = function(request, index) {
    ...
    return function() {
      if(!isValidRequest(request)) {
        responses[index] = utils.response(self.errorResponse(Server.errors.INVALID_REQUEST));
        maybeRespond();
      } else {
