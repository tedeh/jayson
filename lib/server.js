...
Server.prototype.call = function(request, originalCallback) {
  ...
  maybeParse(request, this.options, function(err, request) {
    ...
    // This should be an array or an object. 
    if(request.params) {
      numParams = (Array.isArray(request.params)) 
      ? numParams = request.params.length 
      : numParams = Object.keys(request.params).length;
    }

    if (numParams != method.handler.length - 1)
    {
      return respond(self.error(Server.errors.INVALID_PARAMS));
    }
