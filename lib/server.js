 ...
 *  @param {Boolean} [options.collect=false] Passed to Jayson.Method as an option when created
...
Server.prototype.method = function(name, definition) {
  ...
  var isFunctionInArray = true;
  if(_.isArray(definition)) {
    for (var i = 0; i < definition.length; i++) {
      if(!_.isFunction(definition[i])) {
        isFunctionInArray = false;
        break;
      }
    }
  }
  ...
  if(!isRelay && !isMethod && !isFunction && !isFunctionInArray) {
    throw new TypeError('method definition must be either a function, an instance of jayson.Client, an instance of jayson.Method, or array of function');
  }
