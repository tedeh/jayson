...
JaysonForkServer.prototype.spawn = function() {
  ...
  this.child.on('message', function(msg) {
    ...
    if(msg.err) {
      ...
      utils.parse(msg.response, self.options.reviver, function(err, response) {
...
JaysonForkServer.prototype.call = function(request, callback) {
  ...
  // we must stringify to support reviving/replacing
  utils.stringify(request, this.options.replacer, function(err, body) {
    if(err) return callback(err);
    self.child.send({
