...
/**
 *  Constructor for a Jayson JSON-RPC Server
 *  @constructor
...
utils.inherits(JaysonServer, events.EventEmitter);

JaysonServer.interfaces = {
  http: require('./http'),
  https: require('./https'),
  middleware: require('./middleware')
};

JaysonServer.fork = require('./fork');
 ...
 *  @param {Function|JaysonClient} definition Function or client relay
...
JaysonServer.prototype.method = function(name, definition) {
  ...
  if(typeof(definition) !== 'function' && !(definition instanceof jayson.Client)) throw new TypeError(definition + ' must be either a function or an instance of JaysonClient');
 ...
 *  @param {Object} methods Methods to add as name => definition pairs
 *  @return {void}
 *  @api public
 */
...
/**
 *  Returns a prepared JSON-RPC compatible error property
 ...
 *  @param {Object|Array|String} request A raw JSON-RPC request. Object for single request, Array for batches and String for automatic parsing
...
JaysonServer.prototype.call = function(request, callback) {
  ...
  if(typeof(request) === 'string') {
    try {
      ...
      var error = this.error(JaysonServer.errors.PARSE_ERROR, null, exception);
  ...
  args.push(function(error, result) {
    ...
    if(error) return respond(self.error(JaysonServer.errors.INTERNAL_ERROR));
 ...
 *  @param {Array} requests An array of batch requests
 *  @param {Function} callback Callback that receives one of two arguments: first is an error and the second a response
 *  @return {void}
 *  @api private
 */
...
// is the passed argument a JSON-RPC notification?
