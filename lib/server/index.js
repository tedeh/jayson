...
Server.errorMessages[Server.errors.INVALID_REQUEST_MAX_BATCH_LENGTH_EXCEEDED] =
  'Invalid request: Maximum batch length exceeded';
...
Server.prototype.method = function (name, definition) {
  ...
  if (!isRelay && !isMethod && !isDefinitionFunction) {
    throw new TypeError(
      'method definition must be either a function, an instance of jayson.Client or an instance of jayson.Method'
    );
  }
...
Server.prototype._batch = function (requests, context, callback) {
  ...
  const maybeRespond = function () {
    ...
    const isDone = responses.every(function (response) {
