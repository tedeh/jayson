...
/**
 * Parses delimiter-separated JSON values from an incoming stream
 * @param {Stream} stream
...
Utils.parseStream = function (stream, options, onRequest) {
  options = options || {};
  const delimiter = typeof options.delimiter === 'undefined' ? '\n' : options.delimiter;
  if (typeof delimiter !== 'string' || delimiter.length === 0) {
    throw new TypeError('delimiter must be a non-empty string');
  }

  const decoder = new StringDecoder(options.encoding || 'utf8');
  let buffer = '';
  let failed = false;
  ...
  stream.on('end', function () {
    ...
    parse();
    if (buffer.length > 0) {
      fail(new Error('Stream ended before the JSON delimiter'));
    }
  });
  ...
  function findDelimiter() {
    ...
    for (let index = 0; index < buffer.length; index++) {
      ...
      } else if (buffer.startsWith(delimiter, index)) {
        return index;
      } else if (character === '"') {
        inString = true;
      }
  ...
  function parse() {
    let index;
    while (!failed && (index = findDelimiter()) !== -1) {
      const frame = buffer.slice(0, index);
      buffer = buffer.slice(index + delimiter.length);
      Utils.JSON.parse(frame, options, function (err, value) {
        if (err) {
          return fail(err);
        }
        onRequest(null, value);
      });
    }
  }
};
...
Utils.JSON.parse = function (str, options, callback) {
  ...
  try {
    obj = JSON.parse.apply(
      JSON,
      [str, reviver].filter((v) => v)
    );
  } catch (err) {
    callback(err);
    return;
...
Utils.JSON.stringify = function (obj, options, callback) {
  ...
  try {
    str = JSONstringify.apply(
      JSON,
      [obj, replacer].filter((v) => v)
    );
  } catch (err) {
    callback(err);
    return;
...
Utils.Request.isNotification = function (request) {
  return Boolean(
    request && !Utils.Request.isBatch(request) && (typeof request.id === 'undefined' || request.id === null)
  );
};
...
Utils.Response.isValidError = function (error, version) {
  version = version === 1 ? 1 : 2;
  return Boolean(
    (version === 1 && typeof error !== 'undefined' && error !== null) ||
      (version === 2 &&
        error &&
  ...
  return Boolean(
    response !== null &&
      ...
      ((version === 2 &&
        ...
        // check id
        (response.id === null || typeof response.id === 'string' || typeof response.id === 'number') &&
