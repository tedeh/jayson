...
Utils.parseStream = function(stream, options, onRequest) {
  ...
  const jsonVerifier = verifier.asStream({jsonStreaming: true});
  ...
  jsonVerifier.on('error', onError);
  stream.on('error', onError);

  // nothing reads the verifier, it is only there to reject invalid JSON
  jsonVerifier.resume();

  stream.pipe(jsonVerifier);
  stream.pipe(parser);
};
