  ...
  describe('instance', function () {
    ...
    describe('error', function () {
      ...
      it('should fill in the error message if not passed one', function () {
        ...
        const _error = server.error(code);
    ...
    describe('event handlers', function () {
      (function () {
        ...
        it(
          'should emit "request" upon a request',
          reqShouldEmit(request, 'request', function (req) {
            should.exist(req);
      ...
      (function () {
        ...
        it(
          'should emit "response" upon a response',
          reqShouldEmit(request, 'response', function (req, res) {
            should.exist(req);
            should.exist(res);
            req.id.should.equal(request.id);
      ...
      (function () {
        ...
        it(
          'should emit "batch" upon a batch request',
          reqShouldEmit(request, 'batch', function (batch) {
            should.exist(batch);
    ...
    describe('batch requests', function () {
      describe('with a version 1.0 server', function () {
        ...
        it('should error when version is 1.0', function (done) {
          const request = [utils.request('add', [1, 1]), utils.request('add', [2, 2])];
      ...
      it('should handle a batch with only notifications', function (done) {
        const request = [utils.request('add', [3, 4], null), utils.request('add', [4, 5], null)];
      ...
      it('should handle mixed requests', function (done) {
        const request = [utils.request('add', [1, 1], null), 'invalid request', utils.request('add', [2, 2])];
      ...
      it('should be able return method invocations in correct order', function (done) {
        const request = [utils.request('add_slow', [1, 1, true]), utils.request('add_slow', [1, 2, false])];
    ...
    describe('call context', function () {
      beforeEach(function () {
        server.method(
          'testContext',
          jayson.Method(
            function (params, context, callback) {
      ...
      it('should pass on context for a batch request', function (done) {
        ...
        const request = [utils.request('testContext', []), utils.request('testContext', [])];
