...
export function test_example_17() {
  const methods = {
    ...
    sumDefault: new jayson.Method(
      function (args: any, done: any) {
        const total = sum(args);
        done(null, total);
      },
      {
...
export function test_MethodPromise() {
  const fn1: jaysonPromise.MethodHandlerType = async function (
    args: jaysonPromise.RequestParamsLike,
    context: object,
    callback: jaysonPromise.JSONRPCCallbackTypePlain
  ): Promise<jaysonPromise.JSONRPCResultLike> {
    return {};
  };

  const fn2: jaysonPromise.MethodHandlerType = function (
    args: jaysonPromise.RequestParamsLike,
    callback: jaysonPromise.JSONRPCCallbackTypePlain
  ) {
    callback(null, {});
...
export function test_Method() {
  const fn1: jayson.MethodHandlerType = function (
    args: jayson.RequestParamsLike,
    context: object,
  ...
  const fn2: jayson.MethodHandlerType = function (
    args: jayson.RequestParamsLike,
    callback: jayson.JSONRPCCallbackTypePlain
  ) {
    callback(null, {});
  ...
  const _methodWithoutContext = new jayson.Method(fn2);
...
export function test_clientBrowser() {
  ...
  client.request(
    'multiply',
    [5, 5],
    function (err?: Error | null, error?: jayson.JSONRPCErrorLike, result?: jayson.JSONRPCResultLike) {
      if (err) {
        throw err;
      }
      console.log(result); // 25
...
export function test_server_and_method_call() {
  ...
  server.call(
    request,
    {},
    function (err?: jayson.JSONRPCResponseWithError | null, result?: jayson.JSONRPCResponseWithResult) {
      if (err) {
...
export function test_websocket() {
  ...
  const server = new jayson.Server();
  const _websocketServer = server.websocket({ wss });

  const ws = new WebSocket('ws://localhost:12345');
  const _websocketClient = jayson.Client.websocket({
...
export async function test_websocket_Promise() {
  ...
  const _result = await websocketClient.request('add', [1, 2, 3]);
}
...
export function test_constructors() {
  ...
  const _client = jayson.Client(server);
  const _method = jayson.Method();
}

export function test_constructors_promise() {
  const server = jaysonPromise.Server();
  const _client = jaysonPromise.Client(server);
  const _method = jaysonPromise.Method();
}
